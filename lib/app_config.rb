...
class ApplicationConfiguration
  ...
  # Create a new ApplicationConfiguration object.  <tt>base_conf_path</tt> is the path to your YAML configuration file.
  # If <tt>aux_conf_paths</tt> are given, their contents are recursively merged with the contents of <tt>base_conf_path</tt>.
  ...
  def initialize(base_conf_path, *aux_conf_paths)
    @base_conf_path = base_conf_path
    @aux_conf_paths = aux_conf_paths
    reload!
  end
  ...
  def reload!
    conf = load_conf_file(@base_conf_path)
    aux_confs = @aux_conf_paths.map { |path| load_conf_file(path) }

    aux_confs.each do |aux_conf|
      conf = recursive_merge(conf, aux_conf)
    end
